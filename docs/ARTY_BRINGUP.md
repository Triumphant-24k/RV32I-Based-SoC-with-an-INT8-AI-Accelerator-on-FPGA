...
Everything runs on the input clock. Slow LEDs use counters, not extra clocks. Reset asserts asynchronously and releases after synchronization and 10 ms of stable button release. A short FPGA initialization sequence also requests reset at startup. UART RX passes through two synchronization registers and only drives an activity indicator; this version has no receive command interface.

| FPGA port | Pin | Purpose |
| --- | --- | --- |
| clk | E3 | 100 MHz, constrained to 10 ns |
| reset_n | C2 | Red reset button / CK_RST, pressed low |
| uart_rx | D10 | USB bridge to FPGA |
| uart_tx | A9 | FPGA to USB bridge |
| led[0] | H5 | Heartbeat: toggles every half second |
| led[1] | J5 | Reset active or recent UART activity |
...
Original `demo` and `cpu` images retain their existing locations in `firmware/hex/`. Board variants `board` and `board_cpu` are built under ignored `build/firmware/`. Tcl validates the selected pair before synthesis and passes their paths to the RTL's `$readmemh()` initialization. ELF, map, disassembly, vendor logs and bitstreams stay under ignored `build/`.
...
2. **Reset:** hold the red button. LED1 should show reset and the heartbeat should restart on release. Repeat several times.
