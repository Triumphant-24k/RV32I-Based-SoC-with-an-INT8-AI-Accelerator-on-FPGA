...
module uart_tb;
    ...
    reg [9:0] expected;
    uart_tx #(.CLOCK_HZ(1000),.BAUD(100)) dut(clk,rst,start,data,tx,busy);
    always #5 clk=~clk;
    initial begin
        ...
        for(byte_no=0;byte_no<256;byte_no=byte_no+1)begin
            ...
            for(bit_no=0;bit_no<10;bit_no=bit_no+1)begin
                for(tick_no=0;tick_no<10;tick_no=tick_no+1)begin
    ...
    initial begin #1000000;$fatal(1,"UART test timeout");end
endmodule
