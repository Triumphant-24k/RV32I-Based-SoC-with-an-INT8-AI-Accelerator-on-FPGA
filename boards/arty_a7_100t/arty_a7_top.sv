...
module arty_a7_top #(
...
)(input wire clk, input wire reset_n, input wire uart_rx,
  output wire uart_tx, output wire [3:0] led);
    ...
    wire external_reset=!reset_n || (|power_on);
    ...
    wire [2:0] soc_led;
    wire unused_acc_busy=soc_led[0];
    ...
    always @(posedge clk or posedge rst) begin
        if(rst) begin
            ...
            // RX is activity-only: no receiver or host-command protocol is implied.
            if(!rx_sync[1] || !uart_tx)activity_count<=ACTIVITY_CYCLES[AW-1:0];
    ...
    endgenerate
    // Four active-high discrete Arty LEDs: heartbeat; reset/UART activity; PASS; FAIL/trap.
    assign led={soc_led[2],soc_led[1],rst || activity_count!=0,heartbeat};
`ifndef SYNTHESIS
